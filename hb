...
if not monitor then
    error("No monitor found! Attach a monitor.", 0)
end
...
local baseURL = "https://raw.githubusercontent.com/robertjojo123/shrek6/refs/heads/main/video_part_"
...
if not quadrantIndex or quadrantIndex < 0 or quadrantIndex > 3 then
    error("Error: Label must be 'comp0', 'comp1', 'comp2', or 'comp3'.", 0)
end
...
local frameWidth, frameHeight = 246, 120  -- Updated quadrant resolution
...
function downloadVideo(index, filename)
    ...
    if response then
        ...
        file.close()
        response.close()
        downloadTime = os.epoch("utc") - startTime
        return true
    else
